...
while True:
    samp, notes, length, cycles, volume, form, result = input ("Sample rate (default: 44100): "), input ("Frequencies: ").split (" "), int (input ("Length: ")), int (input ("Cycles: ")), int (input ("Volume: ")), input ("Waveform: "), open ("output.wav", "w")
    notes = [freqs [names.index (i)] if i in names else float (i) for i in notes]
    if samp:
        samp = int (samp)
    else:
        samp = 44100
    orig = notes
    result.setparams ((1, 1, samp, samp * length, "NONE", "not compressed"))
    for m in range (1, cycles + 1):
        for k, j in enumerate (notes):
            wave = []
            for i in range (1, length + 1):
                wave.extend (gen_wave (form, int (samp / j * i - len (wave)), volume))
            if k == 0:
                output = wave
            else:
                output = [sum (l) // 2 for l in zip (output, wave)]
        notes = [k + 1 / (orig [-1] / k) * m for k in orig]
        result.writeframes (bytes (output))
    result.close ()
    input ("Done!")
